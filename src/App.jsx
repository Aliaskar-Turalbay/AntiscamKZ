...
// Данные для интерактивного графика киберугроз в РК
  const chartData = [
    { name: 'Янв', угроз: 1850 },
    { name: 'Фев', угроз: 2100 },
    { name: 'Мар', угроз: 3800 }, // Всплеск (8 марта)
    { name: 'Апр', угроз: 2400 },
    { name: 'Май', угроз: 2900 },
    { name: 'Июн', угроз: 4100 }  // Летние отпуска / фейковые бронирования
  ];
...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState(true); // По умолчанию стильный темный режим
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSearchResult({ 
          ...
          totalComplaints: data.length // Динамически считаем жалобы из базы
        });
      } else {
        setSearchResult({ 
          score: 0, 
      ...
      setLoading(false); // Очищен лишний дублирующийся блок finally
    }
  };
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 relative flex flex-col justify-between overflow-x-hidden ${
      ...
      {/* КИБЕРНЕОНОВЫЙ ЗАДНИЙ ФОН (GLOW EFFECTS) */}
        ...
        <main className="max-w-6xl mx-auto px-4 py-8">
             ...
             {/* RISK SCORE РЕЗУЛЬТАТ */}
                  ...
                  <div className="space-y-4 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                    {searchResult.status !== 'safe' ? (
                      <div>
                        ...
                        {/* АВТОМАТИЧЕСКИЙ ШАБЛОН ДЛЯ Е-ОТИНЫШ */}
              ...
              <div className={`border rounded-3xl p-6 sm:p-8 shadow-inner ${
                ...
                <form onSubmit={handleAddScammer} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Данные:</label>
                      <input 
                        type="text" 
                        placeholder="Телефон, карта или домен" 
                        value={newValue} 
                    ...
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5">Город / Село:</label>
                      <input 
                        type="text" 
                        placeholder="Напр: с. Кордай" 
                        value={newLocation} 
              ...
              {/* Часто задаваемые вопросы (ТЕПЕРЬ В САМОМ НИЗУ) */}
          ...
          {/* ПРАВАЯ КОЛОНКА (Сайдбар) */}
            <div className="space-y-6">
              ...
              {/* ГРАФИК ДИНАМИКИ КИБЕРУГРОЗ (NEON AREA CHART) */}
              <div className={`border rounded-3xl p-5 backdrop-blur-md shadow-sm ${
                ...
                <div className="h-44 w-full -ml-4 pr-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                      <defs>
                        {/* Создаем красивый неоновый градиент для графика */}
                ...
                <p className="text-[10px] text-slate-500 text-center mt-2 font-medium">
                  *Пики зафиксированы в марте (8 марта) и июне (период отпусков).
                </p>
              </div>
              ...
              <div className={`border rounded-3xl p-5 backdrop-blur-md shadow-sm ${
                ...
                <div className="w-full bg-slate-950/20 dark:bg-slate-950/60 rounded-2xl p-2 border dark:border-slate-900 flex justify-center items-center relative group overflow-hidden">
                  <svg viewBox="0 0 500 300" className="w-full h-auto transition-all">
                    <path 
                      d="M20,120 L130,80 L180,140 L160,230 L50,220 Z" 
                      className={`transition-colors cursor-pointer stroke-slate-800/40 stroke-2 ${regionStats.West.color} hover:fill-amber-500/50`}
                      onMouseEnter={() => setSelectedRegion('West')}
                      onMouseLeave={() => setSelectedRegion(null)}
                    />
                    <path 
                      d="M130,80 L280,60 L320,110 L220,150 L180,140 Z" 
                      className={`transition-colors cursor-pointer stroke-slate-800/40 stroke-2 ${regionStats.North.color} hover:fill-yellow-500/50`}
                      onMouseEnter={() => setSelectedRegion('North')}
                      onMouseLeave={() => setSelectedRegion(null)}
                    />
                    <path 
                      d="M180,140 L220,150 L320,110 L380,190 L260,230 Z" 
                      className={`transition-colors cursor-pointer stroke-slate-800/40 stroke-2 ${regionStats.Center.color} hover:fill-orange-500/50`}
                      onMouseEnter={() => setSelectedRegion('Center')}
                      onMouseLeave={() => setSelectedRegion(null)}
                    />
                    <path 
                      d="M320,110 L450,110 L480,190 L380,190 Z" 
                      className={`transition-colors cursor-pointer stroke-slate-800/40 stroke-2 ${regionStats.East.color} hover:fill-yellow-500/50`}
                      onMouseEnter={() => setSelectedRegion('East')}
                      onMouseLeave={() => setSelectedRegion(null)}
                    />
                    <path 
                      d="M160,230 L260,230 L380,190 L420,260 L240,280 L120,270 Z" 
                      className={`transition-colors cursor-pointer stroke-slate-800/40 stroke-2 ${regionStats.South.color} hover:fill-red-500/60`}
                  ...
                  <div className={`absolute bottom-2 left-2 right-2 p-2.5 rounded-xl border text-[11px] font-semibold backdrop-blur-md transition-all duration-300 shadow-xl ${
                    selectedRegion ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'
      ...
      {/* МОДАЛЬНОЕ ОКНО ГЕНЕРАТОРА ЗАЯВЛЕНИЙ */}
      {showLegalModal && (
          ...
          <div className={`w-full max-w-2xl rounded-3xl p-6 border shadow-2xl relative max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            ...
            <div className="relative">
              ...
              <button 
                ...
                className="absolute top-3 right-3 flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-black bg-blue-600 hover:bg-blue-700 text-white transition-all shadow-md active:scale-95"
              >
            ...
            <div className="flex gap-3 mt-5">
              ...
              <a 
                href="https://eotinish.kz" 
                target="_blank" 
                rel="noreferrer"
                className="flex-1 font-black py-3.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs tracking-wider uppercase transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-600/10"
              >
